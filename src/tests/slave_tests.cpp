...
// This is an end-to-end test that verifies that the slave returns the
// correct container status based on the currently running executors, and
// the values returned by the /containers endpoint are as expected.
TEST_F(SlaveTest, ContainersEndpoint)
{
  ...
  NetworkInfo* netInfo = containerStatus.add_network_infos();

  NetworkInfo_IPAddress* ipAddr = netInfo->add_ip_addresses();
